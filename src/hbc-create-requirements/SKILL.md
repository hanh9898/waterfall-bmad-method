...
3. **③ Deep brainstorming — OPTIONAL (B1-7).** A mandatory stop, not the model's call: offer `bmad-brainstorming` with **two seed suggestions** (a *topic* = weakest area, an *output* = what to produce), or go straight to generation. Picked → pause + resume (1a detects the partial). Headless: skip.
