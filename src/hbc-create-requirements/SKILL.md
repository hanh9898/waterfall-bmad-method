...
description: "Generate D-02 Requirements Specification with REQ-xxx IDs. Use when user says 'requirements', 'yêu cầu', '要件定義', or agent menu [REQ]."
---
...
Generate D-02 要件定義書 (Requirements Specification) — structured requirements with unique REQ-xxx IDs, scope boundaries, user roles, functional and non-functional requirements. REQ IDs are the foundation of the entire traceability chain.
...
When `--headless`: all stages run non-interactively. Source documents are required via args. Returns JSON with `status` (`complete` or `blocked`), `output_path`, and validation summary. On `blocked`, includes `reason`.
...
1c. **Intent gate.** Confirm user wants to create/update requirements (not a different artifact). If wrong skill, suggest the right one.
...
Populate `{workflow.template_path}` with discovered content. Ensure:
...
**Parallel-lens menu:** After generation, offer `[A]` Advanced Elicitation (deeper probing on weak areas) / `[P]` Party Mode (multi-agent lateral review) / `[C]` Continue.
...
Finalize document — update frontmatter (`stepsCompleted`, `lastStep = complete`, `updated`). Append closing session to decision log.
