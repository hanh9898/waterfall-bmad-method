...
flowchart LR
    ...
    TR["🔗 Traceability (xuyên suốt)<br/>TRI → TRU → TRA · SYNC · RBL"]
    TR -.->|theo dõi| P1 & P2 & P3 & P4
```

- **Phase 0 (`PI`)** — **bắt buộc, chạy trước tiên**; thường đúng một lần cho cả dự án (hoặc chạy lại để update trực tiếp). **Brownfield** (có code sẵn): document codebase trước (`bmad-document-project` + `project-context.md`) rồi rút deliverable dùng chung từ đó; greenfield: tạo từ PRD/lựa chọn. Sinh D-12, D-03, **constitution** (các nguyên tắc bất biến xuyên phase) + baseline D-19/D-21; idempotent, không cần `feature`.
- **Phase Gate (`PG`)** — chốt kiểm soát ở ranh giới mỗi phase (kiểm tra tự động + đánh giá bằng LLM); mang theo `feature=`. Gate không chỉ pass/fail: có thể **RECYCLE** — trả quyền điều khiển về một phase sớm hơn khi một node thượng nguồn lỗi thời.
...
- **`[RBL]` `hbc-rebaseline`** — engine xuyên feature: re-baseline theo **blast-radius** khi một model dùng chung / cốt lõi đổi sau Phase 3 (mức layout epic/baseline-change trên feature; dry-run → apply).
- **Trục-A — máy cưỡng chế đồng bộ (facts), LLM không quyết** — một **build-graph** làm nguồn-bản-ghi (ground-truth) và ma trận chỉ là **VIEW** suy ra từ đó; máy bắt cứng lệch model/thiếu truy vết (reconcile invariant-FAIL), còn phán đoán "sai có thực chất không" thuộc tầng review LLM.
...
| Xuyên suốt | — | `PG` (Phase Gate) · `TRI`/`TRU`/`TRR`/`TRA` (Traceability) · `SYNC` (Cascade Sync) · `RBL` (`hbc-rebaseline`) | per-feature (`PG`/`TR*`) · cross-cutting (`SYNC`/`RBL`) |
