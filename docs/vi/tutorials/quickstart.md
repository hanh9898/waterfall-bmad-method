...
>
> 📘 **Tutorial** — đi từ con số 0 đến chiến thắng đầu tiên: thấy agent chào bạn và tạo ra tài liệu D-02 đầu tiên. Khoảng 10 phút.
>
> 📖 **Gặp từ lạ?** (deliverable, D-02, phase gate, traceability…) → [Glossary khái niệm](../reference/concept-glossary.md).

## Bạn sẽ đạt được gì

Cuối bài này bạn sẽ: cài xong HBC, xác nhận nó chạy, gõ lệnh đầu tiên, và **có một file D-02 Requirements ra lò** — đủ để biết "à, mình dùng được HBC rồi". Muốn đi trọn 4 phase? Sang [Bắt đầu với HBC (walkthrough)](getting-started-hbc.md) sau khi xong bài này.

## Bước 0 — Chuẩn bị

| Cần có | Kiểm tra |
| --- | --- |
| **AI coding agent** (Claude Code, Cursor, hoặc tương đương) | Đây là nơi bạn sẽ "gõ" các lệnh `BA`, `REQ`… — **không phải** terminal thường |
...
Trạng thái: chưa có deliverable nào trong _bmad-output/ → bạn đang ở đầu Phase 1.
Gợi ý: mở Business Analyst (gõ BA), rồi tạo D-02 Requirements (gõ REQ).
```

> ℹ️ Nếu bản cài của bạn không có `bmad-help`, cứ bỏ qua bước này và gõ thẳng `BA` ở dưới — nó là phép thử đủ để xác nhận HBC đã sẵn sàng.
...
## Bước 3 — Tạo tài liệu đầu tiên (D-02)
...
Agent sẽ phỏng vấn bạn về một yêu cầu. Cứ trả lời ngắn gọn, ví dụ:
...
**Kết quả mong đợi** — một file **D-02 Requirements Specification** xuất hiện trong `_bmad-output/planning-artifacts/`, chứa các yêu cầu đánh mã `REQ-001`, `REQ-002`… Mở file đó ra xem: đó chính là deliverable đầu tiên của bạn.

> 🎉 **Bạn đã hoàn thành Quickstart!** Bạn vừa: cài HBC → xác nhận chạy → tạo D-02. Đây là một mốc **dừng được** — bạn đã biết cách dùng HBC ở mức cơ bản.

## Bước tiếp theo

- ▶️ **Đi trọn vòng đời:** [Bắt đầu với HBC (walkthrough 4 phase)](getting-started-hbc.md) — từ D-02 đến nghiệm thu.
- 🗺️ Xem toàn cảnh: [Bản đồ quy trình](workflow-map.md).
- 💡 Hiểu khái niệm: [Khái niệm cốt lõi](../explanation/concepts.md).
