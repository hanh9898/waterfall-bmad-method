...
>
> ℹ️ **Lưu ý về `PG 1`:** D-02 chưa đủ để qua phase gate Phase 1. Để pass `PG 1` còn cần **D-03 (từ Phase 0, dùng chung)**, **D-06 (BFD — sơ đồ luồng nghiệp vụ, per-feature, bắt buộc)**, và mục **`P1-09` model-validation** (bạn ký xác nhận domain model đã được kiểm chứng). Luồng Phase 1 đầy đủ nằm trong [Bắt đầu với HBC](getting-started-hbc.md).
